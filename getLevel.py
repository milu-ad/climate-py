...
class Graph():  # 图 => 由顶点所构成的图
    ...
    def addEdge(self, f, t, cost=1):  # 添加一条边.
        ...
        if f not in self.nodeSet:
            ...
            #     print('1_104',f, t, len(self.totalNodes['1_104']))
...
#     dir = os.getcwd() + '\data\level_combine_ini'
#     lists = os.listdir(dir)  # 列出文件夹下所有的目录与文件
#     chartDtPath = os.getcwd() + '\data\chartData_ini'
#
#     for i in range(0, len(lists)):
#         path = os.path.join(dir, lists[i])
#         for root, dirs, files in os.walk(path):
#             province = root.split('\\')[-1]
#             dir_province = chartDtPath +'/' + province
#             isExists = os.path.exists(dir_province)
#             if not isExists:
#                 os.makedirs(dir_province)
#             # p =0
...
#                         g.addEdge(data[j],data[j+1])
#                 # print(g.getVertices() ) # 原始序列
#                 print(g.getVertex(g.getVertices()[0]))
#                 #  0 connectedTo: [2, 17]
#
...
#                     print('v',v.getId(),curNode,nextNode)
#                     if nextNode:
#
#                     s_key = str(list(curNode.keys())[0])
#                     t_key = str(list(curNode.keys())[0])
#                     s_value = str(list(nextNode.values())[0])
#                     t_value = str(list(nextNode.values())[0])
...
#                         r_key =s_key+'_'+s_value+'*'+t_key+'_'+t_value
#
...
#                         tmp = []
#                         tmp.append([g.nodeLists[v.getId()],g.nodeLists[w.getId()]] )
#                         tmp.append( v.getWeight(w))
#                         res.append(tmp)
#                         # print("({},{},weight:{})".format(g.nodeLists[v.getId()], g.nodeLists[w.getId()], v.getWeight(w)))
#
#                 f1 = open(chartDtPath+'/'+province+'/'+city+'.json','w',encoding='utf-8')
        ...
        for root, dirs, files in os.walk(path):
            ...
            for file in files:
                ...
                with open(dir_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    for j in range(0, len(data) - 1):
                        # print(data[j], data[j + 1])  # 末尾一条 {'2': 1} {'1': 126}
                        g.addEdge(data[j], data[j + 1])
                        # if j == len(data)-2:
                        #     print("out",j)
                ...
                for v in g:  # 循环类实例 => return ->  g = VertextObject的集合  v = VertextObject
                    count+=1
                    print(count,g.nodeLists[v.getId()])  # {'2': 5}
                    nodeCount = 0
                    curNode = g.getVertices()[v.getId()]  # v {'2': 5}
                    nodeLength = len(g.nodeLists)
                    nextNode = (v.getId() < nodeLength - 1) and (g.getVertices()[v.getId() + 1])
                    ...
                    for w in v.getConnections():  # 获得类实例的connectedTO
                        ...
                        # print(w,v.getWeight(w))
                        # for index_t in range(0,v.getWeight(w)):
                        #     link_tmp = {}
                        #
                        #     source_tmp = {'group': s_groupName, 'name': g.totalNodes[s_groupName][index_t+pre_count], 'value': 1}
                        #     target_tmp = { 'group': t_groupName,'name': g.totalNodes[t_groupName][index_t], 'value': 1}
                        #
                        #     link_tmp['source'] = source_tmp
                        #     link_tmp['target'] = target_tmp
                        #     link_tmp['value'] = 1
                        ...
                        # print(links)
                        print('r_key',r_key,g.totalNodes[t_groupName])
                        if r_key not in sequenceRecord.keys():
                            reverse_key = t_key + '_' + t_value + '*' + s_key + '_' + s_value
                            if reverse_key in sequenceRecord.keys():
                                sequenceRecord[reverse_key].append(v.getWeight(w))
                                ...
                                sequenceRecord[r_key] = [v.getWeight(w)]
                                for index_t in range(0, v.getWeight(w)):
                                    print(t_groupName, v.getWeight(w),v,index_t,g.totalNodes[t_groupName][index_t])
