                    ...
                    for index in range(0,end_index):
                        ...
                        for t in range(len_t):
                            for k, v in data[index+t].items():
                                source.append(k)
                                count += v
                        sec_distance = DTW(source, target,'dtwd')[-1,-1]  # 2,3,4,5
                        peroid_obj.append(start_count)
                        peroid_obj.append(count)

                        if sec_distance not in result:
                            res_distance_store.append(sec_distance)
                            result[sec_distance] = {}
                        ...
                        source = []
                        count = start_count
    min = heapq.nsmallest(3, res_distance_store)
    # nlargest 最大的元素
...
if __name__ == "__main__":
    ...
    u = '2017-01-1'
    t = time.strptime(u+' 00:00:00', '%Y-%m-%d %H:%M:%S')
    print(t.tm_yday)
    # print(get_date(2016, 1))

    # s1 = [2,5,2,4,3]
    # # query_level(s1)
    # get_secquence('上海市',1,6)
